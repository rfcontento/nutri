...
class LocationProvider {
  ...
  Future<LocationModel> initLocationService(
      {bool estimatedPosition = false,
        bool forceLocationManager = false,
        int timeoutTime = 30, bool requestPermission = true,}) async {
    try {
      ...
      if (!serviceEnabled) {
        return LocationModel(LocationStatus.disabled, null);
      }
      ...
      if (permission == LocationPermission.denied) {
        if(requestPermission) {
          ...
          return LocationModel(LocationStatus.denied, null);
        }
      ...
      if (permission == LocationPermission.always ||
        ...
        if (estimatedPosition) {
          ...
          if (lastPosition != null) {
            ...
            initPositionUpdates();
            position = lastPosition;
            return LocationModel(LocationStatus.success, lastPosition);
          } else {
            ...
            if (position?.latitude == 0.0 && position?.longitude == 0.0) {
              return LocationModel(LocationStatus.failed, lastPosition);
            }
            lastPosition = position;
            initPositionUpdates();
            return LocationModel(LocationStatus.success, position);
          }
        } else {
          bool timeout = false;
          ...
          if (position != null &&
            ...
            initPositionUpdates();
            if (aproximate) {
              return LocationModel(LocationStatus.aproximate, position);
            }
            return LocationModel(LocationStatus.success, position);
          }
          if (timeout) {
            return LocationModel(LocationStatus.timeout, position);
          }
        }
      } else {
        return LocationModel(LocationStatus.denied, null);
      ...
      return LocationModel(LocationStatus.failed, null);
    }
    return LocationModel(LocationStatus.failed, null);
  }
  ...
  Future<LocationModel> verifyLocationStatus() async {
    try {
      serviceEnabled = await Geolocator.isLocationServiceEnabled();
      if (!serviceEnabled) {
        return LocationModel(LocationStatus.disabled, null);
      ...
      initPositionUpdates();
      return LocationModel(LocationStatus.success, null);
    } on Exception catch (_) {
      return LocationModel(LocationStatus.failed, null);
    }
  }
